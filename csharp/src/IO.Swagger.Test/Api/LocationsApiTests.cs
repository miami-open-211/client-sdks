/* 
 * Miami Open 211 API
 *
 * This is a developer area for the Miami Open211 API, providing access to the raw data behind the Switchboard of Miami website. The purpose of this API is to allow anyone to develop web, mobile, or other types of applications that serve the mission of Miami Open211.
 *
 * OpenAPI spec version: 1.0.0
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

using System;
using System.IO;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.Reflection;
using RestSharp;
using NUnit.Framework;

using IO.Swagger.Client;
using IO.Swagger.Api;
using IO.Swagger.Model;

namespace IO.Swagger.Test
{
    /// <summary>
    ///  Class for testing LocationsApi
    /// </summary>
    /// <remarks>
    /// This file is automatically generated by Swagger Codegen.
    /// Please update the test case below to test the API endpoint.
    /// </remarks>
    [TestFixture]
    public class LocationsApiTests
    {
        private LocationsApi instance;

        /// <summary>
        /// Setup before each unit test
        /// </summary>
        [SetUp]
        public void Init()
        {
            instance = new LocationsApi();
        }

        /// <summary>
        /// Clean up after each unit test
        /// </summary>
        [TearDown]
        public void Cleanup()
        {

        }

        /// <summary>
        /// Test an instance of LocationsApi
        /// </summary>
        [Test]
        public void InstanceTest()
        {
            // TODO uncomment below to test 'IsInstanceOfType' LocationsApi
            //Assert.IsInstanceOfType(typeof(LocationsApi), instance, "instance is a LocationsApi");
        }

        
        /// <summary>
        /// Test FindNearbyLocations
        /// </summary>
        [Test]
        public void FindNearbyLocationsTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //string accept = null;
            //string locationId = null;
            //string radius = null;
            //string page = null;
            //string perPage = null;
            //var response = instance.FindNearbyLocations(accept, locationId, radius, page, perPage);
            //Assert.IsInstanceOf<List<Location>> (response, "response is List<Location>");
        }
        
        /// <summary>
        /// Test RetrieveLocation
        /// </summary>
        [Test]
        public void RetrieveLocationTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //string accept = null;
            //string locationId = null;
            //var response = instance.RetrieveLocation(accept, locationId);
            //Assert.IsInstanceOf<List<Location>> (response, "response is List<Location>");
        }
        
        /// <summary>
        /// Test RetrieveLocationServices
        /// </summary>
        [Test]
        public void RetrieveLocationServicesTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //string accept = null;
            //string locationId = null;
            //var response = instance.RetrieveLocationServices(accept, locationId);
            //Assert.IsInstanceOf<List<Service>> (response, "response is List<Service>");
        }
        
        /// <summary>
        /// Test RetrieveLocations
        /// </summary>
        [Test]
        public void RetrieveLocationsTest()
        {
            // TODO uncomment below to test the method and replace null with proper value
            //string accept = null;
            //string page = null;
            //string perPage = null;
            //var response = instance.RetrieveLocations(accept, page, perPage);
            //Assert.IsInstanceOf<List<Location>> (response, "response is List<Location>");
        }
        
    }

}
